...
// of this Tetris MVC framework. Both the local player and the optional remote
// player maintain their own Board object.

function Board(tscreen) {
  ...
  this.next_piece = function() {
    ...
    for (var sq = 0; sq < 4; sq++) {
      pos = shapes[this.next_shape][0][sq];
  ...
  this.collides = function(x, y, r) {
    var xx, yy;
    for (var sq = 0; sq < 4; sq++) {
      xx = x + shapes[this.shape][r][sq][0];
      yy = y + shapes[this.shape][r][sq][1];
  ...
  this.drop_piece = function() {
    var xx, yy, sq;
    if (this.collides(this.px, this.py + 1, this.rot)) {
      for (sq = 0; sq < 4; sq++) {
        xx = this.px + shapes[this.shape][this.rot][sq][0];
        yy = this.py + shapes[this.shape][this.rot][sq][1];
  ...
  this.draw_board = function() {
    ...
    for (sq = 0; sq < 4; sq++) {
      p = shapes[this.shape][this.rot][sq];
  ...
  this.add_attack_lines = function(lines) {
    var x, y;
    if (this.py > 0)
      this.py--;
  ...
  this.peer_update = function(update) {
